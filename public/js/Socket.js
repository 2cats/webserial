...
class Socket extends React.Component {
  constructor(props) {
    ...
    this.socket.on('rx', function(data) { this._onData(data); }.bind(this));
    ...
    //   this._onControl({type: 'connected', value: this.connected})
    // }.bind(this));
    this.socket.on('serial_connected', function(data) {
      console.log(data);
      if(data==='y'){
        Notification.success("Serial Port Connected");
        this.connected = true;
      }else if(data==='n'){
        Notification.error("Serial Port Disconnected");
        this.connected = false;
      }
      this._onControl({type: 'connected', value: this.connected})
    }.bind(this));
  ...
  QueryHistory(name) { this.socket.emit('history', name); }
  TruncateLog() {
  ...
  onData(func) { this.onDataFuncs.push(func); }
  onControl(func) {
